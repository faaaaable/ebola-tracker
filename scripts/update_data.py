...
def rebuild_reports_list(current_reports):
    ...
    Si plusieurs fichiers PDF correspondent au même numéro de sitrep
    (ex. un ancien upload manuel "SITREP_MVEBDB_086_....pdf" à côté du
    téléchargement automatique "SITREP_MVE_086.pdf"), un seul est
    conservé — en priorité celui qui a le nom le plus "standard" produit
    par download_all_sitreps.py, sinon le plus récemment modifié."""
    existing_by_num = {r["sitrepNumber"]: r for r in current_reports}
    all_pdfs = glob.glob(os.path.join(REPORTS_DIR, "*.pdf"))
    ...
    for pdf_path in all_pdfs:
        ...
        if num not in by_num:
            by_num[num] = pdf_path
            continue
        # Doublon détecté pour ce numéro : on choisit lequel garder.
        current = by_num[num]
        current_is_standard = os.path.basename(current).startswith("SITREP_MVE_")
        candidate_is_standard = os.path.basename(pdf_path).startswith("SITREP_MVE_")
        if candidate_is_standard and not current_is_standard:
            chosen = pdf_path
        elif current_is_standard and not candidate_is_standard:
            chosen = current
        else:
            # même statut : on garde le plus récemment modifié
            chosen = pdf_path if os.path.getmtime(pdf_path) > os.path.getmtime(current) else current
        ignored = pdf_path if chosen != pdf_path else current
        print(f"  ! doublon pour le sitrep {num} : {os.path.basename(current)} vs "
              f"{os.path.basename(pdf_path)} -> conservé : {os.path.basename(chosen)} "
              f"(ignoré : {os.path.basename(ignored)})")
        by_num[num] = chosen

    reports = []
    for num, pdf_path in by_num.items():
...
def update_sitreps_curve(meta, national):
    """Ajoute/actualise le point du jour dans data/sitreps.json, qui
    alimente la courbe épidémique historique du site (indépendant de
    data/latest.json)."""
    ...
    entry = {
        "date": date,
        "confirmed": national.get("confirmed"),
        "deaths": national.get("deaths"),
        "recovered": national.get("recovered"),
    }
    points = [p for p in points if p.get("date") != date]
    points.append(entry)
...
def main():
    ...
    curve_points = update_sitreps_curve(meta, national)
    ...
    print(f"data/sitreps.json mis à jour : {len(curve_points)} point(s) sur la courbe.")
    return 0
