...
# Le taux de létalité contient parfois un espace parasite avant la virgule
# (ex: "24 ,9" au lieu de "24,9" — artefact d'extraction PDF sur certaines
# lignes) : [\d,\s]+ le tolère, sans risque d'ambiguïté puisque c'est le
# dernier groupe de la ligne (rien à confondre avec un champ suivant).
PROVINCE_LINE_RE = re.compile(
    r"^(?P<prov>ITURI|NORD-KIVU|SUD-KIVU|HAUT-UÉLÉ|TSHOPO|BAS-UÉLÉ)\*?\s+"
    r"(?P<cases>\d+)\*?\s+(?P<deaths>\d+)\*?\s+[\d,\s]+%?\*?\s*$"
)
...
# ce qui s'est produit avec Nia-Nia sur plusieurs SitRep de fin juin, et
# avec Bunia le 2 juillet (espace parasite dans le taux de létalité).
ZONE_LINE_RE = re.compile(
    r"^(?P<name>[A-Za-zÀ-ÖØ-öø-ÿ][A-Za-zÀ-ÖØ-öø-ÿ'\.\- ]*?)\*?\s+"
    r"(?P<cases>\d+)\*?\s+(?P<deaths>\d+)\*?\s+[\d,\s]+%?\*?\s*$"
)
TOTAL_LINE_RE = re.compile(r"^TOTAL\*?\s+(?P<cases>\d+)\*?\s+(?P<deaths>\d+)\*?\s+[\d,\s]+%?\*?\s*$")
